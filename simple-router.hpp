...
namespace simple_router
{
   ...
   */
    ...
    void
    handleARPPacket(const Buffer &packet, const std::string &inIface);
    void
    handleIPPacket(const Buffer &packet, const std::string &inIface);
    void
    sendICMPPacket(const Buffer &packet, ICMPTYPE type);
    void
    forwardPacket(Buffer &packet); // Forward IP packet
    void
    addWaitingPacket(const Buffer &packet); // Add waiting packet
    void
    ticker(); // Ticker

  private:
    std::list<Buffer> m_waitingPackets; // Queue packet waiting to send
    volatile bool m_shouldStop;         // Thread should stop
    std::thread m_tickerThread;         // Thread
