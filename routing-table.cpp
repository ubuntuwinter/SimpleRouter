...
namespace simple_router
{
  ...
  RoutingTable::lookup(uint32_t ip) const
  {
    ...
    // FILL THIS IN
    const RoutingTableEntry *longest_matched_entry = nullptr; // Top-const pointer
    for (const auto &entry : m_entries)
    {
      if ((entry.mask & entry.dest) == (entry.mask & ip))
      {
        if (longest_matched_entry == nullptr || entry.mask >= longest_matched_entry->mask)
        // By compare two mask to judge max profix match
        {
          longest_matched_entry = &entry;
        }
      }
    }
    if (longest_matched_entry)
    {
